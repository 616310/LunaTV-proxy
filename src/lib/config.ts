...
function decodeBase58String(encoded: string): string {
  ...
  for (const char of sanitized) {
    ...
    for (let i = 0; i < decoded.length; i++) {
      const value = decoded[i] * base + carry;
  ...
  const result = Uint8Array.from(decoded.reverse());
  return new TextDecoder().decode(result);
}

const DEFAULT_CONFIG_CANDIDATES = [
  process.env.DEFAULT_CONFIG_FILE?.trim(),
  path.join(process.cwd(), 'config', 'default-config.json'),
  path.join(process.cwd(), 'config', 'default-config.base58'),
].filter((filePath): filePath is string => Boolean(filePath));

let embeddedConfigCache: string | null | undefined;

function tryParseConfigContent(raw: string, filePath: string): string | null {
  ...
  const looksLikeJson =
    filePath.endsWith('.json') || trimmed.startsWith('{') || trimmed.startsWith('[');
  ...
  if (looksLikeJson) {
    try {
      ...
      console.warn(`默认配置 JSON 解析失败 (${filePath}):`, error);
      return null;
    }
  }

  try {
    const decoded = decodeBase58String(trimmed);
    JSON.parse(decoded);
    return decoded;
  } catch (error) {
    console.warn(`默认配置 Base58 解析失败 (${filePath}):`, error);
    return null;
  }
}

function loadEmbeddedConfig(): string | null {
  if (embeddedConfigCache !== undefined) {
    return embeddedConfigCache;
  }
  ...
  for (const candidate of DEFAULT_CONFIG_CANDIDATES) {
    ...
    try {
      ...
      const parsed = tryParseConfigContent(content, resolvedPath);
      if (parsed) {
        embeddedConfigCache = parsed;
        console.log(`已加载默认配置: ${resolvedPath}`);
        return parsed;
      }
    } catch (error) {
      console.warn(`读取默认配置失败 (${resolvedPath}):`, error);
    }
  }

  embeddedConfigCache = null;
  return null;
}
...
async function getInitConfig(configFile: string | undefined, subConfig: {
  URL: string;
  AutoUpdate: boolean;
  LastCheck: string;
} = {
  ...
  let configFileContent =
    typeof configFile === 'string' && configFile.trim().length > 0
      ? configFile
      : '';
  ...
  if (!configFileContent) {
    ...
    if (embeddedConfig) {
      configFileContent = embeddedConfig;
    }
  }

  let cfgFile: ConfigFileStruct;
  try {
    cfgFile = configFileContent
      ? (JSON.parse(configFileContent) as ConfigFileStruct)
      : ({} as ConfigFileStruct);
  } catch (e) {
