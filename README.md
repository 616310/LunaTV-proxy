...
仓库内置 `config/default-config.base58`（来自 [vodtv/api](https://github.com/vodtv/api) 的 `jin18.txt`，不包含 18+ 源）。当存储里还没有任何配置时，LunaTV 会自动解码这个文件并填充全部资源站点，实现即开即用。

如需更新或替换源，可以：

1. 将 `config/default-config.base58` 替换为最新的 Base58 文件（例如改成 `jingjian.txt` 以包含 🔞 源）。
2. 或者设置 `DEFAULT_CONFIG_FILE` 环境变量，让它指向任意 JSON/Base58 配置文件（支持绝对路径，也支持相对项目根目录的路径）。

该文件只会在“当前没有配置”时生效，后续依旧可以在后台界面自由编辑/覆盖这些源。
...
| DEFAULT_CONFIG_FILE                 | 自定义初始配置文件，当 Config 为空时会自动导入 | 本地 JSON/Base58 文件路径         | config/default-config.base58                                                                                            |
